...
class ReplaceLinkService extends Component
{
    ...
    public function replaceUrl(string $url, LanguageEntry $language, bool $evenExcluded = true): string
    {
        ...
        if (isset($parsedUrl['path']) && ($parsedUrl['path'] !== '' && $parsedUrl['path'] !== '0') && !str_ends_with($parsedUrl['path'], '/')) {
            $parsedUrl['path'] .= '/';
        }
        ...
        if (isset($parsedUrl['path']) && ($parsedUrl['path'] !== '' && $parsedUrl['path'] !== '0')) {
            $rebuiltUrl .= $parsedUrl['path'];
        }
        if (isset($parsedUrl['query']) && ($parsedUrl['query'] !== '' && $parsedUrl['query'] !== '0')) {
            $rebuiltUrl .= '?' . $parsedUrl['query'];
        }
        if (isset($parsedUrl['fragment']) && ($parsedUrl['fragment'] !== '' && $parsedUrl['fragment'] !== '0')) {
