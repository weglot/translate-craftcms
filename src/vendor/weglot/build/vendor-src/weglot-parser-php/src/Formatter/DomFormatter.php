...
if (!\function_exists('array_column') && !\function_exists('Weglot\Vendor\array_column')) {
    /**
     * @param array<int, array<int|string, mixed>> $input
...
class DomFormatter extends AbstractFormatter
{
    ...
    public function handle(array $nodes, &$index): void
    {
        ...
        for ($i = 0; $i < \count($nodes); ++$i) {
            ...
            if (null !== $currentTranslated) {
                $this->metaContent($currentNode, $currentTranslated, $translatableAttributes, $originalWords, $translatedWords);
     ...
     * @param array<int, mixed>    $originalWords
     * @param array<int, mixed>    $translatedWords
     */
    protected function metaContent(array $details, $translated, $translatableAttributes, $originalWords, $translatedWords): void
    {
