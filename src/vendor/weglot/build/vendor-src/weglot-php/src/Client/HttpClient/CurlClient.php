...
namespace Weglot\Vendor\Weglot\Client\HttpClient;
...
class CurlClient implements ClientInterface
{
    ...
    protected function executeRequestWithRetries(array $options, $absUrl)
    {
        ...
        $bypass = \defined('WP_PROXY_BYPASS_HOSTS') ? array_map('trim', explode(',', WP_PROXY_BYPASS_HOSTS)) : [];
        ...
        while (\true) {
            ...
            if (!\in_array($urlHost, $bypass, \true)) {
    ...
    private function sleepTime($numRetries)
    {
        ...
        $sleepSeconds = min(self::INITIAL_NETWORK_RETRY_DELAY * 1.0 * 2 ** ($numRetries - 1), self::MAX_NETWORK_RETRY_DELAY);
