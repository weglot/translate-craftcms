...
namespace Weglot\Vendor\WGSimpleHtmlDom;
...
function file_get_html($url, $use_include_path = \false, $context = null, $offset = -1, $maxLen = -1, $lowercase = \true, $forceTagsClosed = \true, $target_charset = WG_DEFAULT_TARGET_CHARSET, $stripRN = \true, $defaultBRText = WG_DEFAULT_BR_TEXT, $defaultSpanText = WG_DEFAULT_SPAN_TEXT)
{
...
function str_get_html($str, $lowercase = \true, $forceTagsClosed = \true, $target_charset = WG_DEFAULT_TARGET_CHARSET, $stripRN = \true, $defaultBRText = WG_DEFAULT_BR_TEXT, $defaultSpanText = WG_DEFAULT_SPAN_TEXT)
{
    $dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
class simple_html_dom_node
{
    ...
    // dump node's tree
    public function dump($show_attr = \true, $deep = 0)
    {
    ...
    public function dump_node($echo = \true)
    {
        $string = $this->tag;
    ...
    public function outertext()
    {
        ...
        // render end tag
        if (isset($this->_[WG_HDOM_INFO_END]) && 0 != $this->_[WG_HDOM_INFO_END]) {
            $ret .= '</'.$this->tag.'>';
        }
    ...
    public function text()
    {
        ...
        switch ($this->nodetype) {
            case WG_HDOM_TYPE_TEXT:
                return $this->dom->restore_noise($this->_[WG_HDOM_INFO_TEXT]);
            case WG_HDOM_TYPE_COMMENT:
                return '';
            case WG_HDOM_TYPE_UNKNOWN:
                return '';
        }
    ...
    public function makeup()
    {
        ...
        foreach ($this->attr as $key => $val) {
            ...
            if (\true === $val) {
                ...
                if (\false !== strpos($key, 'wg-')) {
                    $quote = '"';
                } else {
                    switch ($this->_[WG_HDOM_INFO_QUOTE][$i]) {
                        case WG_HDOM_QUOTE_DOUBLE:
                            $quote = '"';
                            break;
                        case WG_HDOM_QUOTE_SINGLE:
                            $quote = '\'';
                            break;
                        default:
                            $quote = '';
    ...
    public function find($selector, $idx = null, $lowercase = \false)
    {
        ...
        for ($c = 0; $c < $count; ++$c) {
            ...
            $head = [$this->_[WG_HDOM_INFO_BEGIN] => 1];
    ...
    protected function seek($selector, &$ret, $lowercase = \false, $direct = \false)
    {
        ...
        if (0 == $end) {
            $parent = $this->parent;
            while (!isset($parent->_[WG_HDOM_INFO_END]) && null !== $parent) {
                --$end;
                $parent = $parent->parent;
            }
            $end += $parent->_[WG_HDOM_INFO_END];
        }
        $index = $this->_[WG_HDOM_INFO_BEGIN] ?? 1;
        for ($i = $index + 1; $i < $end; ++$i) {
            ...
            if ('*' === $tag && !$key) {
                if (\in_array($node, $this->children, \true)) {
                    $ret[$i] = 1;
                }
                continue;
            }
            ...
            if ($pass && $key) {
                ...
                } elseif ('plaintext' != $key && !isset($node->attr[$key])) {
                    $pass = \false;
                }
            }
            // compare value
    ...
    protected function parse_selector($selector_string)
    {
        ...
        foreach ($matches as $m) {
            ...
            [$tag, $key, $val, $exp, $no_key, $direct] = [$m[1], null, null, '=', \false, \false];
            if (!empty($m[2])) {
                $key = 'id';
                $val = $m[2];
            }
            ...
            if ($this->dom->lowercase) {
                $tag = null !== $tag ? strtolower($tag) : '';
                $key = null !== $key ? strtolower($key) : '';
            }
            ...
            $result[] = [$tag, $key, $val, $exp, $no_key, $direct];
            if (',' === trim($m[7])) {
                $selectors[] = $result;
                $result = [];
            }
    ...
    public function __set($name, $value)
    {
        switch ($name) {
            case 'outertext':
                return $this->_[WG_HDOM_INFO_OUTER] = $value;
            case 'innertext':
                if (isset($this->_[WG_HDOM_INFO_TEXT])) {
    ...
    public function convert_text($text)
    {
        ...
        if (!empty($sourceCharset) && !empty($targetCharset) && 0 != strcasecmp($sourceCharset, $targetCharset)) {
            ...
            if (0 == strcasecmp($targetCharset, 'UTF-8') && $this->is_utf8($text)) {
                $converted_text = $text;
            } else {
    ...
    */
    ...
    public function hasAncestorAttribute($attribute)
    {
        $currentNode = $this;
...
class simple_html_dom
{
    ...
    protected $optional_closing_tags = ['tr' => ['tr' => 1, 'td' => 1, 'th' => 1], 'th' => ['th' => 1], 'td' => ['td' => 1], 'li' => ['li' => 1], 'dt' => ['dt' => 1, 'dd' => 1], 'dd' => ['dd' => 1, 'dt' => 1], 'dl' => ['dd' => 1, 'dt' => 1], 'p' => ['p' => 1], 'nobr' => ['nobr' => 1], 'b' => ['b' => 1], 'option' => ['option' => 1], 'source' => ['source' => 1]];

    public function __construct($str = null, $lowercase = \true, $forceTagsClosed = \true, $target_charset = WG_DEFAULT_TARGET_CHARSET, $stripRN = \true, $defaultBRText = WG_DEFAULT_BR_TEXT, $defaultSpanText = WG_DEFAULT_SPAN_TEXT)
    {
        if ($str) {
    ...
    public function load($str, $lowercase = \true, $stripRN = \true, $defaultBRText = WG_DEFAULT_BR_TEXT, $defaultSpanText = WG_DEFAULT_SPAN_TEXT)
    {
        global $debugObject;
        // prepare
    ...
    {
        $args = \func_get_args();
        $this->load(\call_user_func_array('file_get_contents', $args), \true);
    ...
    public function dump($show_attr = \true)
    {
        $this->root->dump($show_attr);
    }

    // prepare HTML data and init everything
    protected function prepare($str, $lowercase = \true, $stripRN = \true, $defaultBRText = WG_DEFAULT_BR_TEXT, $defaultSpanText = WG_DEFAULT_SPAN_TEXT)
    {
        $this->clear();
    ...
    protected function parse_charset()
    {
        global $debugObject;
        $charset = null;
        if (\function_exists('Weglot\Vendor\get_last_retrieve_url_contents_content_type')) {
